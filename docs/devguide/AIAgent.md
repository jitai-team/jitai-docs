...
### Agent调用页面函数
页面中的内置函数（例如获取页面变量）、页面中组件的函数（例如表格刷新、打开弹窗）以及开发者自定义的[页面函数](门户与页面开发/组件化页面开发#页面函数)都可以作为工具被Agent调用，从而实现了AI对前端界面的操作。
...
开发者在工具函数右侧可以看到`设置权限`按钮，点击后在弹窗中选择[应用角色](/docs/devguide/用户与权限/角色权限)即可，支持选择多个应用角色。
...
[如何在页面中集成AI助理？](门户与页面开发/组件化页面开发#在页面中集成ai助理)
