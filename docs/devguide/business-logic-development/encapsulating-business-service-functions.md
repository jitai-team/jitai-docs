...
## 服务函数在哪里使用
在[页面函数](../portal-and-page-development/component-based-page-development#页面函数)、[页面事件](../portal-and-page-development/component-based-page-development#页面事件)、[前端组件事件函数](../portal-and-page-development/component-based-page-development#事件面板)、[任务函数](./background-tasks#任务执行函数开发)、[模型事件](./event-handling#模型事件)、[审批事件](./event-handling#审批事件)、[自定义事件](./event-handling#自定义事件)、[ai-assistant事件](./event-handling#ai-assistant事件)、[Agent工具调用事件](./event-handling#agent工具调用事件)以及其它服务函数等所有可以编写函数逻辑的地方，开发者都可以调用服务函数。

服务函数还可以[作为ai-agent的工具](../ai-agent#agent调用服务函数)被大模型调用、在[ai-assistant的函数调用节点](../ai-assistant#函数调用)被调用、在[API授权元素](../api-open/api-authorization)中开放给第三方调用。
...
## 使用跨App服务元素调用授权接口
当一个JitAi应用使用[API授权元素](../api-open/api-authorization)对外开放了服务函数，另一个JitAi应用可以使用`跨APP服务`元素访问该授权函数，实现跨应用服务函数调用。这种调用方式仅限于在JitAi应用之间进行，调用方不是JitAi应用则依然需要使用[API授权元素SDK调用方式](../api-open/api-authorization#使用sdk调用授权的接口)。
...
如上图所示，在按钮的点击事件逻辑中发起对跨App服务元素中的函数调用，并使用前端全局提示工具显示函数返回的内容。这里仅仅是用按钮事件函数举例，开发者可以在任意函数逻辑中调用，参考[服务函数在哪里使用](#服务函数在哪里使用)。
