...
- **到达节点时**：AI Agent节点、函数调用节点、条件分支节点及多任务执行节点在到达节点时触发。前提是需要节点开启触发后端事件。开启方式见：<a href="../ai-assistant#节点运行事件-1" target="_blank">节点运行事件</a>。携带的参数是节点上配置的`到达节点事件输出参数`。
...
## Agent工具调用事件
AI Agent在调用工具时，我们可以通过订阅该事件在Agent运行过程中插入一些业务处理逻辑。前提是工具需要开启<a href="ai-agent开发#ai-agent工具配置">触发事件</a>。
