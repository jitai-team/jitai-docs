...
Consider the `Employee Information Table` as an example. Initially designed to store basic employee information, as business requirements evolve, you may need to include dimensional data such as annual sales figures and project participation counts for each employee. However, this information is not directly available in the `Employee Information Table`. Using the extension table model, you can flexibly associate the `Employee Information Table` with related data tables such as `Sales Records Table` and `Project Participation Table`, dynamically extending statistical fields like `Annual Sales` and `Project Count`. This approach allows you to centrally display and analyze employees' multi-dimensional business data in the extension table without altering the original table structure, meeting complex data integration and analysis requirements.

The system provides developers with a convenient extension table creation entry in the development area, supporting streamlined creation, configuration, and real-time editing of extension tables. Through visual operations, developers can quickly complete extension table creation, field configuration, and association relationship setup, significantly improving modeling efficiency and flexibility. All configuration changes are previewed in real-time.

![Extension Table Creation](./img/extended-table-create.png "Extension Table Creation")

In the element tree of the development area, locate `Data Models`, click the `+` button on the right side, and select `Extension Table Model` from the popup list to open the `Create Element > Extension Table Model` dialog.

![Extension Table Dialog](./img/extended-table-popup.png "Extension Table Dialog")

In the `Create Element > Extension Table Model` dialog, fill in the extension table name (the system will automatically generate the English name), select a baseline table, and click `Confirm` to complete the extension table creation and automatically enter the visual editing interface.

![Extension Table Visualization](./img/extended-table-visualization.png "Extension Table Visualization")

In the extension table visual editor, developers can flexibly configure the structure and fields of the extension table while previewing data effects in real-time, facilitating quick validation and design adjustments.

## Connection design {#connection-design}

On the visual editing page of the extension table, switch to the `Connection Design` tab to flexibly configure association relationships between the extension table and the baseline table as well as other data tables. You can directly select the baseline table and set filter conditions to limit the data range, or add other associated tables and configure matching conditions, data filtering, and sorting rules. All established table connections support adjustment or deletion at any time. After properly designing connection relationships, the extension table can efficiently integrate multi-table data to meet multi-dimensional analysis and modeling requirements. Each change in connection configuration is reflected in real-time in the data preview window on the right, helping you intuitively verify configuration results.

### Setting baseline table filter conditions {#setting-baseline-table-filter-conditions}
...
![Data Filtering](./img/data-filtering.gif "Data Filtering")
...
### Adding data tables {#adding-data-tables}
...
![Add Related Table](./img/add-related-table.gif "Add Related Table")
...
### Real-time editing of statistics table configuration {#real-time-editing-statistics-table-configuration}
...
![Edit Statistics Table](./img/edit-statistics-table.gif "Edit Statistics Table")
...
![Field Statistics](./img/extended-table-field-statistics.gif "Field Statistics")
...
### Adding formula fields {#adding-formula-fields}
...
![Extension Table Add Calculation Formula](./img/extended-table-add-calculation-formula.gif "Extension Table Add Calculation Formula")
...
### Modifying field aliases {#modifying-field-aliases}
...
![Extension Table Modify Fields](./img/extended-table-modify-fields.gif "Extension Table Modify Fields")
...
![Extension Table Function Design](./img/extended-table-function-design.png "Extension Table Function Design")
...
![Extension Table Create Function](./img/extended-table-create-function.gif "Extension Table Create Function")
...
### Source code viewing and editing {#source-code-viewing-and-editing}
...
![Extension Table View Source Code](./img/extended-table-view-source-code.gif "Extension Table View Source Code")
