...
JitAi为开发者提供了可视化和全代码双模式的应用开发工具(后续简称IDE)，在[节点控制台](./application-development-basics/runtime-environment-management#节点的本地默认运行环境)中点击"开发"按钮或在应用内切换到“开发者门户”均可进入应用开发界面，IDE会自动加载所有应用模块的源码，开发者可以同时进行可视化编辑和源码编辑，这些应用模块在JitAi技术体系中叫做“元素”。
