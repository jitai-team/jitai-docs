...
When creating an organizational structure, select `DingTalk Self-built Org` to bring up the `Create Element > DingTalk Self-built Org.` dialog. You need to fill in the organization name (English name will be automatically generated), Enterprise ID (CorpID), AgentID, AppKey, AppSecret, and other DingTalk application configuration information. This configuration information is used to establish a secure connection with the DingTalk platform, ensuring the accuracy and security of data synchronization.
...
Each organizational structure supports setting a leader. After clicking the `Set Organization Manager` button, a `Select User` window will pop up. You can filter from all members, select by department or role, or directly choose yourself as the leader for quick selection.
