...
在上图示例中，`echo`函数调用了`sayHello`函数，并将`访问类型`页面变量的值作为入参传递给了`sayHello`函数。
...
![页面函数中调用前后端工具和服务](./img/页面函数中调用前后工具和服务.gif)
