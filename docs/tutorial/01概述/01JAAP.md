...
**JAAP（JitAi Ai Application Protocol）** 是 JitAi 平台的核心协议规范，定义了应用系统的构建标准和模块化架构。协议由 **应用规范** 和 **元素规范** 两部分组成，共同构建了一个完整的应用开发和运行体系。
...
- **目录结构**：与源码目录保持一致
- **文件格式**：元素文件经过加密处理
- **清单文件**：`app.jit` 基于 `app.json` 自动生成，包含编译时信息
:::
