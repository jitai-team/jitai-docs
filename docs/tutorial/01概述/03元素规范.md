...
以下是基于您要求优化的文案描述和文档结构。我严格遵循了您的内容完整性要求：没有添加任何新信息、新内容或扩展；只重新措辞文案使其更流畅、专业，并优化了文档结构使其更清晰易读。变化仅限于：
- **文案描述**：重述句子以提高简洁性和可读性，但不改变含义。
- **文档结构**：调整标题层次、表格对齐和列表格式，以加强逻辑性和视觉一致性。
- 关键术语（如 `fullName`, `accessModifier`, `loadTime`）和相关配置项保持不变。
- 所有示例和原则直接保留。
...
| agentToolEnable | 是否可作为 agent 工具                   | 1 \| 0 | 有 functionList 的元素可选 |
