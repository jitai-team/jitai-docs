...
class Loader(BaseLoader):
    def load(self):
        """加载元素"""
        # self.nodes 是继承链上的元素object列表，node[0] 是当前元素
        node = self.nodes[0]
        return node
```
...
在极态的元素代码中通过`app.getElement(fullName)`获取元素object，然后调用元素的方法。
...
Type元素开放可配置的参数并定义配置读取的方式，实例元素提供配置文件，Type元素的loader加载配置文件后构造实例元素的元素实体object。
...
每个元素对应一个独立的目录。fullName 是通过将元素的相对路径中的目录分隔符替换为"."而得到的string。目录中包含一个 e.json 文件，用于描述该元素的配置。
