            ...
            with self.element.env.getReqContext(self.element.appId):
                ...
                if action == "feedback":
                    ...
                    card_data = {
                        "config": {"autoLayout": True, "enableForward": True},
                        "header": {
                            "title": {"type": "text", "text": "JitAI 智能助手"},
                            "logo": "@lALPDfJ6V_FPDmvNAfTNAfQ",
                        },
                        "contents": [
                            {"type": "markdown", "text": original_response, "id": f"text_{int(time.time() * 1000)}"},
                            {"type": "divider", "id": f"divider_{int(time.time() * 1000)}"},
                            {
                                "type": "action",
                                "actions": [
                                    ...
                                    },
                                    {
                                        ...
                                        "value": json.dumps({"action": "continue_chat", "message_id": message_id}),
                                    },
                                ],
                                "id": f"action_{int(time.time() * 1000)}",
                            },
                            {"type": "text", "text": "感谢您的反馈！", "id": f"text_tips_{int(time.time() * 1000)}"},
                        ],
                    }

                    self.reply_card(card_data=card_data, incoming_message=incoming_message, at_sender=True)
                    ...
                    self.reply_card(card_data=final_card_data, incoming_message=incoming_message, at_sender=True)

                elif action == "continue_chat":
                    # 继续对话
                    self.reply_text("请继续输入您的问题，我会继续为您解答。", incoming_message)
                else:
