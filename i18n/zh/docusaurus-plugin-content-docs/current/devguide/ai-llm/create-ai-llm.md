...
# 创建大模型元素
大模型元素是JitAI平台统一接入各类AI大模型服务的核心组件。它为平台中的[AI Agent](../ai-agent/overview)、[AI助理](../ai-agent/create-flow-agent)、[AI知识库](../knowledge-base/create-knowledge-elements)提供底层模型能力支撑。
...
平台支持对模型列表进行自定义管理。在大模型列表区域，可以勾选需要启用的模型，点击`编辑`按钮可配置模型的高级参数（如temperature、max_tokens等）。点击右上角的`+`按钮可添加自定义模型，支持配置模型名称、类型（对话/向量/重排序）及JSON格式的参数。自定义模型会置顶显示，并可随时删除。这对于接入新发布的模型或私有化部署的模型特别有用。
