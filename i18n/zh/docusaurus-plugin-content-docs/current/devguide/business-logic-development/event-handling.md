...
### 任意/指定字段触发 {#any-specific-field-triggers}
...
- **到达节点时**：AI Agent节点、函数调用节点、条件分支节点及多任务执行节点在到达节点时触发。前提是需要节点开启触发后端事件。开启方式见：<a href="../ai-assitant/create-ai-assistant#node-runtime-events" target="_blank">节点运行事件</a>。携带的参数是节点上配置的`到达节点事件输出参数`。
