...
在服务的`service.py`函数逻辑中, 导包：`from events.Meta import JitEvent` ，使用`JitEvent.publish` 触发事件，其中参数`sender`表示事件的发送者，通常sender为函数路径，`args`用于传递参数。
...
## 用服务函数替换事件内函数 {#replacing-event-internal-functions-with-service-functions}
