...
JitAI平台支持多种登录认证方式，包括账号密码登录、手机号登录、钉钉自建二维码登录、企业微信自建二维码登录、飞书自建二维码登录、微信登录、微信公众号登录、微信小程序登录、GitHub登录、Google登录和Microsoft Teams认证，满足不同场景下用户的接入和认证需求，助力企业和个人用户便捷、安全地访问应用。
...
## 飞书自建二维码登录 {#feishu-custom-qr-login}

飞书自建二维码登录是基于飞书平台的企业级身份认证方式。用户可通过飞书客户端二维码授权，安全便捷地登录系统。该方式适用于企业内部员工统一身份认证、免密登录等场景，有效提升登录体验和安全性。在创建飞书自定义组织架构时，系统会自动为该组织架构生成飞书自建二维码登录方式。开发者也可根据实际需要，独立新增飞书自建二维码登录认证方式。

![飞书二维码扫描](./img/auth/feishu-qr-scan.gif)

当开发者选择创建`飞书自建二维码登录`时，将弹出`新建飞书自建二维码登录`配置窗口。在该窗口中，需填写登录方式名称（英文名自动生成），配置飞书自定义应用的凭证信息，包括`App ID`和`App Secret`。相关配置信息获取方法，请参考[组织架构文档](./organization#feishu-custom-organization)。

![飞书登录配置](./img/auth/feishu-login-config.png)

完成相关配置后，系统将自动创建飞书二维码登录方式，并跳转到登录方式配置页面，供您后续查看和管理。

:::tip
飞书自建二维码登录方式需要配合[飞书自定义组织架构](./organization#feishu-custom-organization)使用。在创建飞书自定义组织架构时，系统会自动同步生成对应的二维码登录方式，无需单独配置。
:::
