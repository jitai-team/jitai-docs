...
# 组织架构
JitAI平台提供灵活多样的组织架构能力，支持常规组织、钉钉自建组织、企业微信自建组织、Microsoft Teams 及飞书自建组织五种类型。用户可根据实际管理需求和系统基础，灵活选择合适的组织架构，实现高效的成员与部门管理。平台不仅支持成员和部门的灵活配置，还可无缝集成第三方平台，全面满足企业多元化的组织管理场景。

## 默认组织架构 {#default-organization}
在创建应用时，JitAI会自动为你生成一个默认的组织架构。

![默认组织架构](./img/corp/default-organization.png)
...
JitAI 支持在同一个应用中创建和管理多个组织架构，满足企业多业务线、跨部门或多子公司的复杂管理需求。开发者可以根据实际场景，灵活选择常规组织、钉钉自建组织或企业微信自建组织等类型，分别配置不同的组织结构、成员、部门及权限体系。每个组织架构相互独立，支持单独的成员管理、部门设置和权限分配，便于实现多租户或多团队协作。通过多组织架构能力，企业可以高效应对组织扩展、业务调整和多平台集成等挑战，提升整体管理效率和系统灵活性。

![组织架构创建](./img/corp/organization-creation.png)

在开发区的元素树中，点击`组织架构`右侧的 `+` 按钮，选择创建常规组织、钉钉自建组织、企业微信自建组织、Microsoft Teams 或飞书自建组织，并填写相关配置信息，完成组织架构的创建。

## 常规组织 {#generic-organization}
常规组织是组织架构元素中最基础的一种，用户可在JitAI中自行维护通讯录信息，不依赖第三方，适用于没有使用第三方通讯办公平台的企业。

![常规组织创建](./img/corp/standard-organization-creation.gif)

在创建组织架构时，选择常规组织架构，即可出现`新建常规组织`弹窗，输入组织名称(英文名会自动生成)，输入组织负责人姓名，手机号（非必填），账号密码（默认**admin123**），是否允许新注册用户加入。

![常规组织1](./img/corp/standard-organization-1.png)

填完配置信息后，点击`确定`即可进入到可视化编辑器中查看。

### 允许新注册用户加入 {#allowing-new-registered-users-to-join}
在创建常规组织架构时，系统会显示`允许新注册用户加入`勾选框，且默认已勾选。勾选后，新的注册用户可直接加入该组织架构，便于成员快速扩展和管理。
...
**钉钉配置信息获取指引：可参考[钉钉开放平台文档](https://open-dev.dingtalk.com)，详细的应用创建和配置步骤请参考[钉钉工作台应用指南](./dingtalk-workspace-app-guide)**
...
**企业微信配置信息获取指引：可参考[企业微信官方文档](https://work.weixin.qq.com)，详细的应用创建和配置步骤请参考[企业微信工作台应用指南](./wechat-work-app-guide)**
...
## 飞书自建组织 {#feishu-custom-organization}
飞书自建组织是为使用飞书（Lark/Feishu）办公平台的企业设计的组织架构元素。通过该类型，可以实现与飞书平台的深度集成，自动同步飞书中的部门结构、成员信息等数据，避免重复维护通讯录的工作负担，为企业提供一体化的办公协作解决方案。

![飞书自建组织创建](./img/corp/feishu-custom-org-creation.gif)

在创建组织架构时，选择飞书自建组织，即可出现`新建飞书自建组织`弹窗。需要填写组织名称（英文名会自动生成）、App ID、App Secret等飞书应用配置信息。这些配置信息用于建立与飞书平台的安全连接，确保数据同步的准确性和安全性。配置完成后，系统将自动从飞书平台同步组织架构、部门信息和成员数据，实现统一的身份认证和权限管理。

**飞书配置信息获取指引：可参考[飞书开放平台文档](https://open.feishu.cn)，详细的应用创建和配置步骤请参考[飞书工作台应用指南](./feishu-workspace-app-guide)**

![飞书应用ID获取示例](./img/corp/feishu-app-key-secret.png)

App ID、App Secret 等应用参数需在飞书开放平台的自建应用详情页中获取。

组织架构创建完成后，JitAI将自动为该组织配置飞书自建扫码登录方式，无需手动设置，便于成员通过飞书扫码快捷登录系统。

:::tip 注意
进行飞书组织同步前，请确保已在飞书开放平台为自建应用分配了以下权限：`获取用户基本信息`、`获取通讯录基本信息`、`以应用身份读取通讯录`等，以保障数据同步的顺利进行。
:::
...
在组织架构管理中，你可以为每个组织设置负责人，明确管理职责。设置负责人有助于在通讯录、审批等场景中快速定位主要联系人，提升组织管理效率。

![设置组织负责人](./img/corp/set-organization-manager.gif)

每个组织架构均支持设置负责人。点击`设置组织负责人`按钮后，将弹出`选择用户`窗口。你可以从全部成员中筛选，也可按部门、角色进行选择，或直接快捷选择自己作为负责人。
...
## 常规组织通讯录 {#generic-organization-contacts}
常规组织通讯录用于集中展示和管理企业的部门、成员、角色等信息，方便开发者进行组织架构调整、成员维护和权限分配。

![常规组织操作](./img/corp/standard-organization-operations.gif)

### 新建部门 {#creating-department}
在常规组织的`组织架构`标签页中，开发者可以根据实际业务需求新建部门，灵活搭建企业的多级组织层级结构。支持为每个部门设置名称、上级部门等信息，便于后续的管理和调整。
...
![常规组织角色操作](./img/corp/standard-organization-role-operations.gif)
...
## 飞书自建组织通讯录 {#feishu-organization-contacts}
界面管理同[钉钉组织架构管理页面](#dingtalk-organization-contacts)
