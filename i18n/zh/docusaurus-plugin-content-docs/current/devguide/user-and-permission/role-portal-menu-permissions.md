...
根据业务流程设计配套的角色权限体系是企业级应用设计的重要一环。JitAI提供多层级立体化地操作权限以及数据权限管控方案。开发者可以围绕`应用角色+门户+菜单+页面组件+Agent+数据模型+服务函数`配置有区分的访问边界，最大限度地保障应用使用期间的数据安全，防止越权访问。

本篇重点介绍门户、菜单、页面组件和角色成员管理。如果需要配置角色可使用的 Agent、数据模型函数和服务函数，请继续阅读[按角色开放 Agent、数据模型和服务函数](./roles-and-business-element-permissions)。
