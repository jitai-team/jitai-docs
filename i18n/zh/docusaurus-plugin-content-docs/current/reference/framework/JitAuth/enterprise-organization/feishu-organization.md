...
description: "飞书自建组织 API 参考文档。完整的规格说明、方法和示例。"
---
# 飞书自建组织
飞书自建组织是JitAuth框架中企业级组织架构集成类型，基于飞书自建应用API实现组织架构同步和用户管理。它负责飞书通讯录同步、组织信息获取和用户身份验证，支持自动组织架构同步和用户权限继承。

飞书自建组织元素分层结构为Meta（corps.Meta） → Type（corps.FSInnerType） → 实例，开发者可通过JitAI的可视化开发工具快捷地创建飞书自建组织实例元素。

当然，开发者也可以创建自己的Type元素，或者在自己的App中改写JitAI官方提供的corps.FSInnerType元素，以实现自己的封装。
...
#### 认证配置文件
```json title="MyFeishuCorp.json配置"
{
...
| type | string | 是 | 固定值：corps.FSInnerType |
...
## 方法 
### initCorp
初始化组织架构，飞书自建的组织架构初始化时会自动同步组织架构数据。
...
| 参数名 | 类型 | 对应原生类型 | 必填 | 说明 |
|--------|------|-------------|------|------|
| corpData | JitDict | dict | 是 | 飞书组织架构数据 |
...
**返回值：** object - 飞书API客户端对象

```python title="获取飞书客户端"
corp = app.getElement("corps.MyFeishuCorp")
...
```python title="获取认证配置"
corp = app.getElement("corps.MyFeishuCorp")
print(f"应用ID: {corp.authConfig['appId']}")
print(f"应用Secret: {corp.authConfig['appSecret']}")
```

## 高级特性
### 自动同步配置
飞书自建组织支持定时自动同步飞书通讯录数据，确保本地组织架构与飞书保持一致。
