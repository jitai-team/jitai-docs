...
运维工具主要负责开发者团队、运行环境以及应用部署管理。在运行环境管理方面，对一个组织内的集群环境进行管理，创建和删除集群，配置集群中部署的应用、服务器节点、分流权重等。同时提供节点管理功能，对一个组织内部署的所有 JitAI 应用平台服务器节点进行远程管理。此外还负责应用管理和开发者团队管理，接受开发者自主注册、加入组织，对一个组织的开发者成员进行管理。
...
![效率曲线对比](img/briefintro/development-efficiency-comparison.svg)
