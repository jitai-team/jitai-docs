...
数据录入页面的创建过程和[常规页面的创建过程](./generic-page)基本一致，不同的是数据录入页面需要绑定一个[数据表模型](../data-modeling/data-table-model)，用于存储用户录入的数据。
...
开发者可以在页面编辑器中点击右上角的`转为常规页面`按钮，即可将页面转换为常规页面。关于常规页面的开发方法参考[组件化页面开发](./generic-page)。
